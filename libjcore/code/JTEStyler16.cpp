/******************************************************************************
 JTEStyler16.cpp
	...
	styles inside a JTextEditor16.  Derive a class from JTextEditor16 and
	override AdjustStylesBeforeRecalc() and pass the arguments directly to
	JTEStyler16::UpdateStyles().

	tokenStartList is used to avoid lexing more of the text than necessary.
	Each JTextEditor16 object must own one and must never modify the contents
	...
	)
{
	...
	std::istrstream input((const char*)text.GetCString(), text.GetLength() * sizeof(JCharacter16));
	...
	#if DEBUG_TIMING_INFO
	timer.StopTimer();
	cout << "JTEStyler16: " << timer.PrintTimeInterval() << endl;
	#endif
...
void
JTEStyler16::PreexpandCheckRange
	(
	const JString16&						text,
	...
	)
{
	...
	if (itsRedoAllFlag)
		...
		itsStyles->AppendElements(JTextEditor16::Font(fid, itsFontSize, style, itsDefFontAlign),
								  range.GetLength());
		}
		...
		JTextEditor16::Font f = itsStyles->GetRunData(itsTokenRunIndex);
...
JArray<JTEStyler16::TokenData>*
JTEStyler16::NewTokenStartList()
{
...
JTEStyler16::TokenExtra
JTEStyler16::GetFirstTokenExtraData()
	const
{
	return TokenExtra();
}
...
JTEStyler16::CompareTokenStarts
	(
	const TokenData& t1,
	const TokenData& t2
	)
	...
	)
{
	...
	*itsRecalcRange += range;
	...
	JTextEditor16::Font f = itsStyles->GetElement(range.first);
	f.size  = itsFontSize;
	f.style = style;
	if (OnlyColorChanged(style, itsDefFontStyle))
		{
...
#define JTemplateType JTEStyler16::TokenData
